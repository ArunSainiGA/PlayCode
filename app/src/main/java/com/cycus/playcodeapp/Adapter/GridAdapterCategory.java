...
//    DynamicSizes objDynamicSizes;
    ...
    public GridAdapterCategory(Context context, ArrayList<GamesBean> arrayList, String catIcon, Activity catActivity ) {
...
//            mlp.height= objDynamicSizes.getHeightOfEachGrid();
            ...
            convertView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    intent.putExtra("cat_id", String.valueOf(arrayList.get(pos_final).getCatId()));
                    intent.putExtra("game_id", String.valueOf(arrayList.get(pos_final).getGameId()));
                    Log.i("Disptch_intent_cat", catIcon);
                    intent.putExtra("cat_icon", catIcon);
                    intent.putExtra("game_name", arrayList.get(pos_final).getGameTitle());
                    intent.putExtra("game_price", String.valueOf(arrayList.get(pos_final).getGamePrice()));
                    intent.putExtra("cat_name", arrayList.get(pos_final).getCatName());
                    intent.putExtra("game_rating", String.valueOf(arrayList.get(pos_final).getGameRating()));
                    intent.putExtra("game_desc", arrayList.get(pos_final).getGameDesc());
                    intent.putExtra("game_thumbnail", arrayList.get(pos_final).getGameThumbnail());
                    intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    context.startActivity(intent);
                }
            });
